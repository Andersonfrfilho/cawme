...
Kong API Gateway (porta 8000)
    │  Rota: /bff/auth/register → upstream bff:3001
    ▼
...
O BFF deve expor `POST /bff/auth/register` que:
...
Serviço BFF habilitado no Kong (`http://bff:3001`). Rota `/bff/auth` com rate limiting 5 req/min + CORS.
...
Endpoint `POST /bff/auth/forgot-password` (documentado no AuthFlow.md, não implementado).
