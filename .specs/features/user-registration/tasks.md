...
**Conteúdo:** `POST /bff/auth/register`, `POST /bff/auth/forgot-password`  
...
- Adicionar rota `bff-auth` (`/bff/auth`, strip_path: false)
...
**Alteração:** Adicionar `REGISTER: "/bff/auth/register"` ao `AUTH_ENDPOINTS`
...
**Conteúdo:** POST `/bff/auth/register` com JSON body. Sem armazenar tokens (não é auto-login).
...
2. POST `http://localhost:3001/bff/auth/register` — verificar 201
